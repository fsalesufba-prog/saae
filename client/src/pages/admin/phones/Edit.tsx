...
const PhonesEdit: React.FC = () => {
  ...
  useEffect(() => {
    const loadPhone = async () => {
      try {
        const response = await api.get(`/useful-phones/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/useful-phones/${id}`, formData);
      navigate('/admin/phones');
    } catch (err: any) {
      setError(err.response?.data?.error || 'Erro ao atualizar telefone');
    } finally {
      setLoading(false);
    }
  };
